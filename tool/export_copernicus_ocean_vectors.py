...
預設 dataset_id（GLOBAL_ANALYSISFORECAST_PHY_001_024 之「Currents, daily」含 uo/vo）：
  cmems_mod_glo_phy-cur_anfc_0.083deg_P1D-m
  （勿用 cmems_mod_glo_phy_anfc_0.083deg_P1D-m：為物理場日平均，不含 uo/vo）
...
GitHub Actions：
  設 CMEMS_USE_COPERNICUSMARINE=1，並在 Secrets 提供 CMEMS_USER／CMEMS_PASSWORD
  （會以 COPERNICUSMARINE_SERVICE_USERNAME／PASSWORD 傳入 copernicusmarine）。

環境變數：
  CMEMS_USE_COPERNICUSMARINE  設為 1 時優先使用 copernicusmarine（需安裝套件）
